...
with tab1:
    ...
    with col2:
        pct_lost = round(total_lost / (total_rev + total_lost) * 100, 1)
        st.markdown(f"""<div class="kpi kpi-amber">
            <div class="kpi-label">Lost as % of potential</div>
            <div class="kpi-value">{pct_lost}%</div>
    ...
    selected_skus = st.multiselect(
        "Filter SKU",
        sales["SKU ID"].unique().tolist(),
        default=sales["SKU ID"].unique().tolist(),
        key="sku_filter1"
    )
    ...
    with col1:
        ...
        for sku in selected_skus:
            ...
            fig.add_trace(go.Scatter(
                x=d["Week Start Date"], y=d["Actual Demand Units"],
                name=f"{SKU_NAMES.get(sku, sku)} Demand",
                line=dict(color=SKU_COLORS.get(sku, "#888"), width=1.5, dash="dot"),
            ))
            fig.add_trace(go.Scatter(
                x=d["Week Start Date"], y=d["Actual Sales Units"],
                name=f"{SKU_NAMES.get(sku, sku)} Sales",
                line=dict(color=SKU_COLORS.get(sku, "#888"), width=2),
            ))
        fig.update_layout(
            height=320, plot_bgcolor="white",
            yaxis=dict(title="Units", gridcolor="#F1F1F1"),
            xaxis=dict(title=""),
    ...
    with col2:
        ...
        for sku in selected_skus:
            ...
            fig2.add_trace(go.Scatter(
                x=normal_d["Week Start Date"], y=normal_d["Actual Sales Units"],
                name=f"{SKU_NAMES.get(sku, sku)} Normal",
                line=dict(color=SKU_COLORS.get(sku, "#888"), width=2),
                mode="lines",
            ))
            if len(stockout_d) > 0:
                fig2.add_trace(go.Scatter(
                    x=stockout_d["Week Start Date"], y=stockout_d["Actual Sales Units"],
                    name=f"{SKU_NAMES.get(sku, sku)} Stockout",
                    mode="markers",
                    marker=dict(color="#C0392B", size=8, symbol="x"),
                ))
        fig2.update_layout(
            height=320, plot_bgcolor="white",
            yaxis=dict(title="Sales Units", gridcolor="#F1F1F1"),
            xaxis=dict(title=""),
            legend=dict(orientation="h", y=1.15, font=dict(size=10)),
            margin=dict(t=10, b=20),
        )
...
with tab2:
    ...
    fig_fc.add_trace(go.Scatter(
        x=sku_data["Week Start Date"], y=sku_data["Actual Demand Units"],
        name="Actual Demand",
        line=dict(color="#111", width=2),
    ))
    fig_fc.add_trace(go.Scatter(
        x=sku_data["Week Start Date"], y=sku_data["4 Week Moving Average"],
    ...
    for _, row in stockout_weeks.iterrows():
        fig_fc.add_vrect(
            x0=row["Week Start Date"], x1=row["Week Start Date"],
            fillcolor="#C0392B", opacity=0.15, line_width=0,
        )
    ...
    with col1:
        ...
        fig_seas = px.line(
            weekly_avg, x="Week Number", y="Actual Demand Units",
            markers=True,
            color_discrete_sequence=[SKU_COLORS.get(sku_select, "#888")],
        )
        fig_seas.update_layout(
            height=280, plot_bgcolor="white",
            yaxis=dict(title="Avg Weekly Units", gridcolor="#F1F1F1"),
    ...
    with col2:
        ...
        fig_promo = px.bar(
            promo_comp, x="Label", y="Actual Demand Units",
            color="Label",
            color_discrete_map={"Promotion Week": "#F39C12", "Normal Week": "#94A3B8"},
            text="Actual Demand Units",
        )
        fig_promo.update_traces(texttemplate="%{text:.0f}", textposition="outside")
        fig_promo.update_layout(
            height=280, plot_bgcolor="white",
            yaxis=dict(title="Avg Units", gridcolor="#F1F1F1"),
            xaxis=dict(title=""),
            showlegend=False,
            margin=dict(t=10, b=20),
        )
...
with tab3:
    ...
    with col1:
        ...
        fig_eoq = px.bar(
            reorder, x="SKU Name", y="EOQ Units",
            color="SKU ID", color_discrete_map=SKU_COLORS,
            text="EOQ Units",
        )
        fig_eoq.update_traces(texttemplate="%{text:,}", textposition="outside")
        fig_eoq.update_layout(
            height=300, plot_bgcolor="white",
            yaxis=dict(title="Units per Order", gridcolor="#F1F1F1"),
            xaxis=dict(title=""),
            showlegend=False,
    ...
    with col2:
        ...
        fig_rop = go.Figure()
        fig_rop.add_trace(go.Bar(
            name="Recommended Reorder Point",
            x=reorder["SKU ID"], y=reorder["Recommended Reorder Point"],
            marker_color="#4285F4",
        ))
        fig_rop.add_trace(go.Bar(
            name="Recommended Safety Stock",
            x=reorder["SKU ID"], y=reorder["Recommended Safety Stock"],
            marker_color="#F39C12",
        ))
...
with tab4:
    ...
    for col, (_, row) in zip([col1, col2, col3], sku_summary.iterrows()):
        with col:
    ...
    with col1:
        ...
        fig_lost = px.bar(
            monthly_lost, x="Month", y="Lost Revenue CAD",
            color="SKU ID", color_discrete_map=SKU_COLORS,
            barmode="stack",
        )
        fig_lost.update_layout(
            height=320, plot_bgcolor="white",
            yaxis=dict(title="Lost Revenue (CAD)", gridcolor="#F1F1F1", tickformat="$,.0f"),
    ...
    with col2:
        st.markdown('<div class="section-title">Stockout weeks vs promotion weeks</div>', unsafe_allow_html=True)
        overlap = sales.groupby(["SKU ID", "Is Promotion Week", "Stockout"]).size().reset_index(name="Count")
        fig_overlap = px.bar(
            overlap, x="SKU ID", y="Count",
            color="Stockout", facet_col="Is Promotion Week",
            barmode="stack",
...
st.divider()
st.markdown(
    "**Data note:** All sales data is synthetic and generated for portfolio purposes. "
    "NorthSnack Foods is a fictional company. Demand patterns are modelled on real CPG seasonal "
    "and promotional dynamics. Prepared by Simran Saran as part of The Case Files portfolio series."
)
